...
function LatestQuestions(props) {
  const { loading, data, error } = useQuery(GET_LATEST_QUESTIONS, {
    pollInterval: 5000,
  });
  ...
  return (
    ...
    >
      <div style={{ width: "100%", padding: "0 auto", margin: "0 auto" }}></div>
      <div
        className="row"
        style={{
          display: "flex",
      ...
      {questions.map((question, index) => {
        ...
        return (
          <ShortQuestion key={index} id={id} text={text} answers={answers} />
        );
      })}
    </div>
  );
